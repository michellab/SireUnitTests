...
def test_props(verbose = False):
    ...
    # load the molecules
    s1 = MoleculeParser.read("../io/ala.top", "../io/ala.crd", \
    ...
    for i in range(0,s1.nMolecules()):
        ...
        for j in range(0,atoms1.count()):
            assert_equal( atoms1[j].property("fish"),
                          atoms2[j].property("coordinates") )

            assert_equal( atoms1[j].property("puppy"),
                          atoms2[j].property("charge") )
