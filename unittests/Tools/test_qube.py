...
def qube(PDB, XML, TOP, CRD, folder, verbose=False):
    
    (molecules_q, space) = readXmlParameters (PDB, XML)
    system_q = createSystem(molecules_q) 
    system_q = setupForcefields(system_q, space)
    rst = Sire.IO.AmberRst7(system_q)
    prm = AmberPrm(system_q)
    pdb_name = '../io/qube/'+folder+'/MOL'
    print("pdb name: ",pdb_name)
    prm.writeToFile("%s.prm7"%pdb_name)
    rst.writeToFile("%s.rst7"%pdb_name)
    nrg_qube = system_q.energy()
    ...
    nrg_amber = system.energy()

    nrg_diff = (nrg_amber - nrg_qube).value()
    if verbose:
        print ("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        print ("* The energy from the Amber files is: %s" %nrg_amber)
        print ("* The energy from the QUBE files is: %s" % nrg_qube)

    # Return the energy difference.
    return nrg_diff


def test_qube(verbose=True):
    for folder in os.listdir('../io/qube/'):
        print("Now working at: ",folder)
        PDB = '../io/qube/'+folder+'/MOL.pdb'
        XML = '../io/qube/'+folder+'/MOL.xml'
        TOP = '../io/qube/'+folder+'/MOL.prm7'
        CRD = '../io/qube/'+folder+'/MOL.rst7'
        
        diff_nrg = qube(PDB, XML, TOP, CRD, folder, verbose=verbose)
        assert_almost_equal(diff_nrg, 0.0, 4)
